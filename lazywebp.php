...
/*
	...
	Description: WordPress plugin that lazyloads images while checking for the existence of webp, when it is active automatically all images you upload have been converted to webp format as well
	Author: Erik
	Version: 0.0.1
    ...
    <script>
      ...
      async function imageExists(url) {
        if (url) return new Promise((resolve, reject) => {
          var image = new Image();
          image.src = url + '.webp';
          if (image.complete) resolve("isComplete");
          image.onload = () => resolve("isLoading");
          image.onerror = () => reject(`unable to locate ${url}.webp`)
        });
      }
      ...
      async function loadImage(elem) {
        if ( elem.classList.contains('lazyloading') || !elem.getAttribute('src') || !elem.complete ) {
          await imageExists(elem.dataset.src)
            .then((res) => {
              elem.classList.add('lazyload');
              // store the file extension
              const fileExt = elem.dataset.src.split('.').pop();
              // hijack the request to webp image format
              elem.src = elem.dataset.src + ".webp";
              elem.srcset = elem.dataset.srcset ? elem.dataset.srcset.replaceAll("." + fileExt, `.${fileExt}.webp`) : '';
              // add a class once the image has been fully loaded
              imageLoaded(elem.dataset.src).then(() => elem.classList.add('lazyloaded'))
            })
            .catch(() => {
              ...
              if (elem.dataset.src) {
                ...
                imageLoaded(elem.dataset.src).then(() => {
            ...
            .then(async () => {
              // clean
              elem.classList.remove('lazyloading');
        ...
        if ('IntersectionObserver' in window) {
          ...
          imgCollection.forEach(image => {
            if (!image.getAttribute('src')) {
              image.onload = () => { image.src = `data:image/svg+xml,%3Csvg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${image.naturalWidth} ${image.naturalHeight}"%3E%3C/svg%3E` }
            }
